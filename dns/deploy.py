...
def set_ip_thread(site, node):
   ...
   if site == 0:
     commands.append("ip route add %s/32 dev eth1" % (topology['fog'][5]['dns'][0]['ip_vlan']))
     commands.append("ip route add %s/32 via %s" % (topology['fog'][1]['dns'][0]['ip_vlan'],topology['fog'][5]['dns'][0]['ip_vlan']))
     commands.append("ip route add %s/32 via %s" % (topology['fog'][2]['dns'][0]['ip_vlan'],topology['fog'][5]['dns'][0]['ip_vlan']))
     commands.append("ip route add %s/32 via %s" % (topology['fog'][3]['dns'][0]['ip_vlan'],topology['fog'][5]['dns'][0]['ip_vlan']))
     commands.append("ip route add %s/32 via %s" % (topology['fog'][4]['dns'][0]['ip_vlan'],topology['fog'][5]['dns'][0]['ip_vlan']))
     commands.append("ip route add %s/32 via %s" % (topology['fog'][6]['dns'][0]['ip_vlan'],topology['fog'][5]['dns'][0]['ip_vlan']))
     commands.append("ip route add %s/32 via %s" % (topology['fog'][7]['dns'][0]['ip_vlan'],topology['fog'][5]['dns'][0]['ip_vlan']))
   if site == 1:
     commands.append("ip route add %s/32 dev eth1" % (topology['fog'][4]['dns'][0]['ip_vlan']))
     commands.append("ip route add %s/32 via %s" % (topology['fog'][0]['dns'][0]['ip_vlan'],topology['fog'][4]['dns'][0]['ip_vlan']))
     commands.append("ip route add %s/32 via %s" % (topology['fog'][2]['dns'][0]['ip_vlan'],topology['fog'][4]['dns'][0]['ip_vlan']))
     commands.append("ip route add %s/32 via %s" % (topology['fog'][3]['dns'][0]['ip_vlan'],topology['fog'][4]['dns'][0]['ip_vlan']))
     commands.append("ip route add %s/32 via %s" % (topology['fog'][5]['dns'][0]['ip_vlan'],topology['fog'][4]['dns'][0]['ip_vlan']))
     commands.append("ip route add %s/32 via %s" % (topology['fog'][6]['dns'][0]['ip_vlan'],topology['fog'][4]['dns'][0]['ip_vlan']))
     commands.append("ip route add %s/32 via %s" % (topology['fog'][7]['dns'][0]['ip_vlan'],topology['fog'][4]['dns'][0]['ip_vlan']))
   if site == 2:
     commands.append("ip route add %s/32 dev eth1" % (topology['fog'][3]['dns'][0]['ip_vlan']))
     commands.append("ip route add %s/32 dev eth1" % (topology['fog'][5]['dns'][0]['ip_vlan']))
     commands.append("ip route add %s/32 via %s" % (topology['fog'][0]['dns'][0]['ip_vlan'],topology['fog'][5]['dns'][0]['ip_vlan']))
     commands.append("ip route add %s/32 via %s" % (topology['fog'][1]['dns'][0]['ip_vlan'],topology['fog'][5]['dns'][0]['ip_vlan']))
     commands.append("ip route add %s/32 via %s" % (topology['fog'][4]['dns'][0]['ip_vlan'],topology['fog'][5]['dns'][0]['ip_vlan']))
     commands.append("ip route add %s/32 via %s" % (topology['fog'][6]['dns'][0]['ip_vlan'],topology['fog'][5]['dns'][0]['ip_vlan']))
     commands.append("ip route add %s/32 via %s" % (topology['fog'][7]['dns'][0]['ip_vlan'],topology['fog'][5]['dns'][0]['ip_vlan']))
   if site == 3:
     ...
     commands.append("ip route add %s/32 via %s" % (topology['fog'][0]['dns'][0]['ip_vlan'],topology['fog'][2]['dns'][0]['ip_vlan']))
     commands.append("ip route add %s/32 via %s" % (topology['fog'][1]['dns'][0]['ip_vlan'],topology['fog'][2]['dns'][0]['ip_vlan']))
     commands.append("ip route add %s/32 via %s" % (topology['fog'][4]['dns'][0]['ip_vlan'],topology['fog'][2]['dns'][0]['ip_vlan']))
     commands.append("ip route add %s/32 via %s" % (topology['fog'][5]['dns'][0]['ip_vlan'],topology['fog'][2]['dns'][0]['ip_vlan']))
     commands.append("ip route add %s/32 via %s" % (topology['fog'][6]['dns'][0]['ip_vlan'],topology['fog'][2]['dns'][0]['ip_vlan']))
     commands.append("ip route add %s/32 via %s" % (topology['fog'][7]['dns'][0]['ip_vlan'],topology['fog'][2]['dns'][0]['ip_vlan']))
   if site == 4:
     commands.append("ip route add %s/32 dev eth1" % (topology['fog'][1]['dns'][0]['ip_vlan']))
     commands.append("ip route add %s/32 dev eth1" % (topology['fog'][5]['dns'][0]['ip_vlan']))
     commands.append("ip route add %s/32 dev eth1" % (topology['fog'][6]['dns'][0]['ip_vlan']))
     commands.append("ip route add %s/32 via %s" % (topology['fog'][0]['dns'][0]['ip_vlan'],topology['fog'][5]['dns'][0]['ip_vlan']))
     commands.append("ip route add %s/32 via %s" % (topology['fog'][2]['dns'][0]['ip_vlan'],topology['fog'][5]['dns'][0]['ip_vlan']))
     commands.append("ip route add %s/32 via %s" % (topology['fog'][3]['dns'][0]['ip_vlan'],topology['fog'][5]['dns'][0]['ip_vlan']))
     commands.append("ip route add %s/32 via %s" % (topology['fog'][7]['dns'][0]['ip_vlan'],topology['fog'][5]['dns'][0]['ip_vlan']))
   if site == 5:
     ...
     commands.append("ip route add %s/32 dev eth1" % (topology['fog'][2]['dns'][0]['ip_vlan']))
     commands.append("ip route add %s/32 dev eth1" % (topology['fog'][4]['dns'][0]['ip_vlan']))
     commands.append("ip route add %s/32 dev eth1" % (topology['fog'][7]['dns'][0]['ip_vlan']))
     commands.append("ip route add %s/32 via %s" % (topology['fog'][1]['dns'][0]['ip_vlan'],topology['fog'][4]['dns'][0]['ip_vlan']))
     commands.append("ip route add %s/32 via %s" % (topology['fog'][6]['dns'][0]['ip_vlan'],topology['fog'][4]['dns'][0]['ip_vlan']))
     commands.append("ip route add %s/32 via %s" % (topology['fog'][3]['dns'][0]['ip_vlan'],topology['fog'][2]['dns'][0]['ip_vlan']))
   if site == 6:
     commands.append("ip route add %s/32 dev eth1" % (topology['fog'][4]['dns'][0]['ip_vlan']))
     commands.append("ip route add %s/32 via %s" % (topology['fog'][0]['dns'][0]['ip_vlan'],topology['fog'][4]['dns'][0]['ip_vlan']))
     commands.append("ip route add %s/32 via %s" % (topology['fog'][1]['dns'][0]['ip_vlan'],topology['fog'][4]['dns'][0]['ip_vlan']))
...
def install_bench():
  ...
  for n in [el for s in topology['fog'] for el in s['client']]+topology['client']:
    ...
    execo.Process('scp /home/bconfais/ipfs+dns/bench_permutation_7_fix.py '+str(n['ip_5k'])+':/tmp/bench_s2.py').run().stdout
...
def deploy_ipfs():
  ...
  for isite, site in enumerate(topology['fog']):
    ...
    commands = [
      ...
      '/tmp/ipfs config DNS.Resolver "%s"' % (topology['fog'][5]['dns'][0]['ip_vlan']),
...
"site7 IN TXT \"%s,%s,%s\"\n" \
"site8 IN TXT \"%s,%s\"\n" \
"" % (topology['fog'][3]['dns'][0]['ip_vlan'], topology['fog'][5]['dns'][0]['ip_vlan'], topology['fog'][0]['dns'][0]['ip_vlan'], topology['fog'][5]['dns'][0]['ip_vlan'], topology['fog'][4]['dns'][0]['ip_vlan'], topology['fog'][1]['dns'][0]['ip_vlan'], topology['fog'][5]['dns'][0]['ip_vlan'], topology['fog'][2]['dns'][0]['ip_vlan'], topology['fog'][5]['dns'][0]['ip_vlan'], topology['fog'][2]['dns'][0]['ip_vlan'], topology['fog'][3]['dns'][0]['ip_vlan'], topology['fog'][5]['dns'][0]['ip_vlan'], topology['fog'][4]['dns'][0]['ip_vlan'], topology['fog'][5]['dns'][0]['ip_vlan'], topology['fog'][5]['dns'][0]['ip_vlan'], topology['fog'][4]['dns'][0]['ip_vlan'], topology['fog'][6]['dns'][0]['ip_vlan'], topology['fog'][5]['dns'][0]['ip_vlan'], topology['fog'][7]['dns'][0]['ip_vlan'],)

  with open('named', 'w') as f:
    f.write(config_)
  execo.Process('scp named '+str(topology['fog'][5]['dns'][0]['ip_5k'])+':/tmp/dns/example.com.zone').run().stdout
...
#    for latency in [10, 5, 133, 168, 312]:
#    for latency in [10, 5, 146, 294, 307]:
    for latency in [1, 4, 5, 7]:
      commands += [
...
def scenario_s2(workload, trial):
  ...
  threads_write.append(
    Thread(
      target=bench_thread,
      args=(('cd /tmp/; python2 bench_s2.py write 0 %dx%dt%d.site1.example.com %d %d %s %s %s %s %s %s %s' % (object['number'], object['size'], trial, workload['number'],workload['size']*1000*1000, str(topology['fog'][0]['storaged'][0]['ip_vlan']), str(topology['fog'][0]['storaged'][0]['ip_vlan']), str(topology['fog'][0]['storaged'][0]['ip_vlan']), str(topology['fog'][0]['storaged'][0]['ip_vlan']), str(topology['fog'][0]['storaged'][0]['ip_vlan']), str(topology['fog'][0]['storaged'][0]['ip_vlan']), str(topology['fog'][0]['storaged'][0]['ip_vlan']), )), topology['client'][0],
      ('%dx%d_trial%d_site%d_client%d_write.txt' % (workload['number'], workload['size'], trial, 1, 1)),)
    )
  )

  # read from another site
  for nbread in range(7):
    threads_read.append([])
    threads_read[nbread].append(
      Thread(
        target=bench_thread,
        args=(('cd /tmp/; python2 bench_s2.py read %d %dx%dt%d.site1.example.com %d %d %s %s %s %s %s %s %s' % (nbread, object['number'], object['size'],trial, workload['number'], workload['size']*1000*1000,  str(topology['fog'][1]['storaged'][0]['ip_vlan']), str(topology['fog'][2]['storaged'][0]['ip_vlan']), str(topology['fog'][3]['storaged'][0]['ip_vlan']), str(topology['fog'][4]['storaged'][0]['ip_vlan']), str(topology['fog'][5]['storaged'][0]['ip_vlan']), str(topology['fog'][6]['storaged'][0]['ip_vlan']), str(topology['fog'][7]['storaged'][0]['ip_vlan']), )), topology['client'][0],
        ('%dx%d_trial%d_site%d_client%d_read%d.txt' % (workload['number'], workload['size'], trial, 1, 1, nbread)),)
      )
    )
...
[5, 5, 5, 5, 5, 5, 5, 1]
  ...
  nodes = []
  with open('/var/lib/oar/%s' %(sys.argv[1]), 'r') as f:
    for node in f.readlines():
      node = node.replace('\n', '')
      ip_vlan, ip = get_ip(node)
      if ip in [c['ip_5k'] for c in nodes]:
        continue
